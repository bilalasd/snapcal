...
> Source: getdesign.md — Figma design.md dark preview
> (`/design-md/figma/preview-dark`).

The monochrome core inverts; the pastel color blocks do **not**. That is the
whole idea — the black-and-white chrome flips to white-on-near-black, but the
`{colors.block-*}` panels keep their exact light-mode hex, so a lime or lilac
block on a `{colors-dark.canvas}` ground is the signature dark moment. Brand
identity is the color blocks, and they are theme-invariant.

### Dark Palette

| Role | Token | Value |
|---|---|---|
| Canvas | `{colors-dark.canvas}` | `#0a0a0a` — default page background |
| Surface 1 | `{colors-dark.surface-1}` | `#161616` — cards, swatches |
| Surface 2 | `{colors-dark.surface-2}` | `#1f1f1f` — elevated tiles, icon buttons |
| Heading | `{colors-dark.ink}` | `#ffffff` |
| Body | `{colors-dark.body-text}` | `rgba(255,255,255,0.85)` |
| Secondary | `{colors-dark.secondary-text}` | `rgba(255,255,255,0.6)` — captions, eyebrows, footers |
| Primary (CTA) | `{colors-dark.primary}` | `#ffffff` — inverted; the black pill goes white |
| On-Primary | `{colors-dark.on-primary}` | `#0a0a0a` — text on white buttons |
| Hairline | `{colors-dark.hairline}` | `rgba(255,255,255,0.12)` — translucent border |
| Magenta Promo | `{colors-dark.accent-magenta}` | `#ff3d8b` — unchanged from light |
| Success | `{colors-dark.semantic-success}` | `#2ecc5b` — brightened for dark-canvas contrast |
| Color blocks | `{colors-dark.block-*}` | lime/lilac/cream/mint/pink/coral/navy — **unchanged** |

### Dark-Mode Rules

- **CTA inversion.** `button-primary` flips to a white pill with `{colors-dark.on-primary}` text; the black/white primary–secondary pair inverts wholesale. Icon buttons use `{colors-dark.surface-2}`.
- **Pastels are brand identity — never darkened.** The `{colors.block-*}` panels keep their exact light-mode hex on the dark canvas, and their ink stays black. Don't tint or dim them for dark mode.
- **Translucent borders replace solid hairlines.** Use `{colors-dark.hairline}` (`rgba(255,255,255,0.12)`) rather than a solid line, so separation reads through opacity.
- **Text is opacity tiers, not new greys.** 100% / 85% / 60% white (`{colors-dark.ink}` / `{colors-dark.body-text}` / `{colors-dark.secondary-text}`), consistent with the light-mode rule that hierarchy comes from weight/opacity, never a mid-grey token.
- **Three elevation steps.** Canvas `{colors-dark.canvas}` → Surface 1 `{colors-dark.surface-1}` (cards) → Surface 2 `{colors-dark.surface-2}` (icon buttons, elevated tiles). The deep navy block (`{colors.block-navy}`) also reads as a surface extension on the dark ground.
...
- Dark mode: the marketing site itself is light-only, but a companion dark theme is now documented in the **Dark Theme** section (sourced from the getdesign.md dark preview). Rule of thumb: invert the monochrome core, keep every pastel color block unchanged.
